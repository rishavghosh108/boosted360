...
export default function PaymentSuccess() {
  ...
  useEffect(()=>{
    const getPaymentStatus = async()=>{
      ...
      await axios.post('http://192.168.1.78:8000/api/paypal/capture',{order_id:token},{
        headers:{
             ...
             <div className="message-box bg-white shadow-lg py-10 px-5 text-center border-b-4 border-primary">
              ...
              {/* {!loggedInUserData && (
                <div className='lg:ml-5 flex items-center justify-center space-x-4 mt-4'>
                  <Link href={menuThemeSettingData?.login_url || ''} className='fill-btn-min w-fit lg:min-w-0 min-w-[120px]'>
                    LogIn
                  </Link>
                </div>
              )} */}
